...
namespace AnalysisSQL
{
	...
	public class SqlParser: ParserBase
	{
        ...
        private XmlNode GetJoinTagXmlNode()
        {
            XmlNode myFromNode = ParsedDocument.SelectSingleNode(string.Format(@"{0}/{1}[@{2}='{3}']", cRootXmlNodeName, cTagXmlNodeName, cTagTypeXmlAttributeName, JoinTag.cTagName));
            if (myFromNode == null)
                throw new Exception(ToText());
        ...
        private XmlNode GetOnTagXmlNode()
        {
            XmlNode myFromNode = ParsedDocument.SelectSingleNode(string.Format(@"{0}/{1}[@{2}='{3}']", cRootXmlNodeName, cTagXmlNodeName, cTagTypeXmlAttributeName, OnTag.cTagName));
            if (myFromNode == null)
                throw new Exception(ToText());
